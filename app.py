# app.py — grouped layout (no markdown code-block issue)
...
# ---------- HTML helper (no leading whitespace/newlines) ----------
def group_html(title: str, body: str, cls: str) -> str:
    # single-line, no indentation -> won't be rendered as a markdown code block
...
if done < total:
    ...
    # Build one compact HTML string — no indentation, no newlines at start
    card_html = "<div class='card'>" + skill_html + ai_html + human_html + "</div>"
    st.markdown(card_html, unsafe_allow_html=True)

    review_html = group_html("📝 Review &amp; Submit", "", "review")
